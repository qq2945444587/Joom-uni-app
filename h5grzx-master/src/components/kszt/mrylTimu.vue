<template>
  <div class="main">
    ...
    <div class="swipe_box">
      ...
      >
        <van-swipe-item v-for="(item, index) in subjectList" :key="index">
...
export default {
  ...
  created() {
    // if (this.comeFrom.choiceList) {
    //   const cuotiList = [];
    //   this.comeFrom.choiceList.forEach((e) => {
    //     if (e.answerCorrectSet === 4) {
    //       cuotiList.push(e.didQuestionIdSet);
    //     }
    //   });
    //   this.mrylList(cuotiList.join(","));
    // } else {
    //   this.dailyPractice(this.subcourseId);
    // }
    this.dailyPractice(this.subcourseId);
  },
  methods: {
    ...
    dailyPractice(val) {
      ...
      StartQuestion.getDailyPracticeBySubCourseId(params)
        .then((res) => {
          if (res.data.V) {
            const data = res.data;
            this.dailyPracticeSetId = data.V.dailyPracticeSetId; // 交卷所需要的每日一练id
            this.$parent.dailyPract(
              this.dailyPracticeSetId,
              data.V.appDailyPracticeSet.questionIdSet
            );
            if (data.V.appDailyPracticeSet.questionIdSet) {
              this.mrylList(data.V.appDailyPracticeSet.questionIdSet);
            }
          }
        })
        .catch((error) => {
